...
export default function QA() {
  ...
  const postPendingAnswer = async (questionID, text, nid, timestamp, token) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/post-answer', {
        method: 'POST',
      ...
      try {
        ...
        if (!alreadyAnswered) {
          await fetch(`http://localhost:4000/update-question-answered/${questionID}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: null,
          });
      ...
      const refreshResponse = await fetch('http://localhost:4000/qas');
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch('http://localhost:4000/qas');
        if (response.ok) {
  ...
  const submitQuestion = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4000/post-question', {
        method: 'POST',
        headers: {
  ...
  const submitAnswer = async (questionID) => {
    ...
    try {
      const toxicResponse = await fetch('http://localhost:4000/check-toxicity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
      const checkResponse = await fetch(
        `http://localhost:4000/check-duplicate-answer?questionID=${questionID}&content=${encodeURIComponent(text)}`,
      );
  ...
  const startTyping = async (typedText) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/get-similar-questions?question=${encodeURIComponent(text)}`,
